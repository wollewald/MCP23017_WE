...
void MCP23017::setPinMode(byte pin, PORT port, STATE state){
		if(port==A){
			if(state==ON){
				ioDirA &= ~(1<<pin);
			}
...
void MCP23017::setPin(byte pin, PORT port, STATE state){
	if(port==A){
		if(state==ON){
			gpioA |= (1<<pin); 
		}
...
void MCP23017::setPinX(byte pin, PORT port, byte ioDir, STATE state){
	if(port==A){
		if(ioDir==OUTPUT){
			ioDirA &= ~(1<<pin);
		}
...
void MCP23017::setAllPins(PORT port, STATE state){
	if(port==A){
		if(state==ON){
			gpioA = B11111111;
		}
...
void MCP23017::setIntOdr(STATE state){
	if(state==ON){
		ioConA |= (1<<INTODR);
		ioConB |= (1<<INTODR);
	}
