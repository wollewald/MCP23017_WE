...
typedef MCP23018_WE MCP; // just for less typing!

/* There are several ways to create your MCP23018 object:
 * MCP23018_WE myMCP = MCP23018_WE(MCP_ADDRESS) -> uses Wire / no reset pin 
 * MCP23018_WE myMCP = MCP23018_WE(MCP_ADDRESS, RESET_PIN) -> uses Wire / RESET_PIN
 * MCP23018_WE myMCP = MCP23018_WE(&Wire, MCP_ADDRESS) -> passing a TwoWire object / no reset pin
 * MCP23018_WE myMCP = MCP23018_WE(&Wire, MCP_ADDRESS, RESET_PIN) -> "all together"
 */
...
void setup(){ 
  ...
  delay(wT);
  myMCP.setAllPins(MCP::A, LOW);            // Port A: all pins are LOW
  myMCP.setAllPins(MCP::B, LOW);            // Port B: all pins are LOW
