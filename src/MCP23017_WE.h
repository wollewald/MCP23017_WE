...
#ifndef MCP23017_WE_H
#define MCP23017_WE_H
...
class MCP23017_WE{
    public:
        ...
        /* constructors */
#ifndef USE_TINY_WIRE_M_
        MCP23017_WE(uint8_t addr = 0x20, uint8_t rp = 99) : _wire{&Wire}, I2C_Address{addr}, resetPin{rp}, useSPI{false} {}
        MCP23017_WE(TwoWire *w, uint8_t addr, uint8_t rp = 99) : _wire{w}, I2C_Address{addr}, resetPin{rp}, useSPI{false} {}
        MCP23017_WE(uint8_t cs, uint8_t rp, uint8_t addr) : _spi{&SPI}, SPI_Address{addr}, resetPin{rp}, csPin{cs}, useSPI{true} {}
        MCP23017_WE(SPIClass *s, uint8_t cs, uint8_t rp, uint8_t addr) : _spi{s}, SPI_Address{addr}, resetPin{rp}, csPin{cs}, useSPI{true} {}
#else
        MCP23017_WE(uint8_t addr, uint8_t rp) : I2C_Address{addr}, resetPin{rp}, useSPI{false} {}
#endif
        ...
        uint8_t getOutputLatch(mcp_port);
        ...
        void setIpolPin(uint8_t, mcp_port, uint8_t);
        void setIpolPort(uint8_t, mcp_port);
        uint8_t getIpolPort(mcp_port);
    ...
    protected:
        ...
        void setOutputLatch(uint8_t, mcp_port);
        void setIoDir(uint8_t, mcp_port);
        uint8_t getIoDir(mcp_port);
        void writeMCP23017_WE(uint8_t, uint8_t);
        void writeMCP23017_WE(uint8_t, uint8_t, uint8_t);
        uint8_t readMCP23017_WE(uint8_t);
#ifndef USE_TINY_WIRE_M_   
        TwoWire *_wire;
        SPIClass *_spi;
